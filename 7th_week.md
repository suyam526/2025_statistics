...
- K-means 클러스터링
    ...
    - k개의 중심점을 찍어서 관측치들 간의 거리를 최적화해 군집화하는 모델
    - 관측치, 중심점 간의 거리를 사용하므로 데이터 표준화 or 정규화 반드시 필요함!!!

    - 구체적인 모델 수행 과정
        1. 데이터 공간에 k개의 중심점을 임의로 찍음
        2. 중심점에서 각 관측치 간의 유클리드 거리를 구함
        3. 중심점과 가까운 관측치들을 군집으로 할당
        4. 군집의 모든 관측치와 중심점 간의 거리 합이 최소가 되도록 중심점을 군집 중앙으로 이동
        5. 이 과정을 중심점이 더 이상 이동하지 않을 때까지 반복(Iteration)

    - 적절한 k는 어떻게 정하나요?
        1. 번째 방법: 비즈니스 도메인 지식을 통한 개수 선정
        2. 번째 방법: 엘보우 기법 
            - 군집 내 중심점과 관측치 간 거리 합
...
## 14.3. 회귀성능 평가지표
<!-- 새롭게 배운 내용을 자유롭게 정리해주세요.-->
- R제곱, RMSE, MAE, MAPE, RMSLE, AIC, BIC

1. R-Square과 Adjusted R-Square
- SSR : 회귀식의 추정값과 전체 실젯값 평균과의 편차 제곱합
- SSE : 회귀식의 추정값과 실젯값 편차 제곱의 합
- SST : 실젯값과 전체 실젯값 평균과의 편차 제곱합

-> R-Square 값은 SSR 값 클수록, SST 값 작을수록 커짐
- SSR 값 크다 = 회귀선이 각 데이터를 고르게 설명한다
- SST 값 작다 = SSR을 제외한 SSE가 작다는 의미

- Adjusted R-Square = R-Square가 독립 변수의 개수가 많아질수록 값이 커지는 문제를 보정한 기준

2. RMSE
- 회귀 모델은 실젯값을 정확히 예측하기 어려우므로 모델의 정확도 파악하는게 까다로움...
- 그래서 수치를 정확히 맞춘 비율 말고, **실제 수치와의 차이**를 지표로 사용

- RMSE : 편차 제곱의 평균에 루트를 씌운 값 (실제 수치와 예측한 수치 간 차이가 평균적으로 어느 정도인지 확인)
- BUT RMSE는 예측값의 스케일에 영향 받음 (즉 종속변수의 단위가 커지면 RMSE도 커진다..ㅎ)
    - 다시 말해서 모델 간 정확도 비교할 때, 표본 데이터가 다르면 RMSE 절대치로 비교하면 안됨 

## 14.6. 유의확률의 함정
<!-- 새롭게 배운 내용을 자유롭게 정리해주세요.-->
1.  p값은 표본의 크기가 커지면 표본 오차가 작아지고, 점점 0으로 수렴하는 특성이 있음
2. p값의 기준인 0.05는 통상적으로 쓰는 임의적 기준이므로 0.05 미만이라고 해서 통계적 유의성 있다고 확언 불가능!!


## 14.7. 분석가의 주관적 판단과 스토리텔링
<!-- 새롭게 배운 내용을 자유롭게 정리해주세요.-->
- 데이터가 무조건 객관적인 진실만을 나타낸다고 생각하면 안됨!!
- 특히 1차원적인 데이터는 사람들의 문화/심리를 나타내지 못하는 경우가 많다
- **분석가는 데이터를 항상 의심해야 한다**!!

올바른 주관적 판단을 위해서는
1. 해당 분야의 도메인 지식 수반
2. 통계적 지식을 기반으로 EDA, 전처리를 성실히 수행
3. 적극적인 커뮤니케이션/검증 과정
...
```
아버지 키가 1cm 증가할 때, 아들의 예상 키가 평균적으로 0.516cm 증가한다는 뜻~
```
---
...
```
Study Hours의 오즈비 2.34는 주당 공부 시간이 1시간 증가할 때, 학생이 ‘낮음’보다 ‘보통’ 또는 ‘높음’ 성취도를 가질 확률이 2.34배 증가함 
Attendance Rate의 오즈비 3.87은 출석률이 1단위(예: 1%) 증가할 때, 학생이 ‘낮음’보다 ‘보통’ 또는 ‘높음’ 성취도를 가질 확률이 3.87배 증가함 

즉 두 변수 모두 학업 성취도를 높이는 데 긍정적인 영향을 미친다고 볼 수 있음
```
...
```
Study Hours: 오즈비가 1.89에서 2.88 사이일 가능성이 95%
Attendance Rate: 오즈비가 2.92에서 5.13 사이일 가능성이 95%

신뢰구간이 1보다 크므로 두 변수 모두 학업 성취도와 유의미한 양의 관계가 있다고 해석할 수 있음
...
```
Attendance Rate의 오즈비가 더 크므로, 출석률이 학업 성취도에 더 큰 영향을 미침

출석률 개선을 위해서 결석 사유를 관리하면 좋을듯!
```
...
1. 엘보우 방법과 2. 실루엣 계수를 함께 사용해서 적절한 k를 찾는 것이 중요함
```
